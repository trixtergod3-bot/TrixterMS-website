...
export const insertAccountSql = 'INSERT INTO accounts (name, password, salt, banned, gm, greason, '
  + 'NxPrepaid, NxCredit, mPoints, points, vpoints, PicEnabled) VALUES (?, ?, ?, 0, 0, 1, 0, 0, 0, 0, 0, 0)';

export function accountStore(pool, rateSecret) {
  return {
    async create(username, record, address) {
      ...
      try {
        ...
        const rows = await connection.query('SELECT id FROM accounts WHERE name = ? LIMIT 1', [username]);
        if (rows.length) { await connection.commit(); return 'duplicate'; }
        try {
          await connection.query(insertAccountSql, [username, record.hash, record.salt]);
        } catch (error) {
          // Only MySQL duplicate-key error; other integrity failures are unavailable.
