...
The proxy allowlists paths and parameters, validates date/name/filter inputs, rejects duplicate parameters, prevents redirect following, bounds every request to four seconds, limits responses to 1 MiB, requires JSON, and reconstructs the public DTO using explicit field allowlists. It returns no raw upstream errors. Status observations older than two minutes fail closed. Responses are not cached. No proxy route accepts SQL, database names, arbitrary URLs, or writable operations.
...
| `/api/rankings` | sort=level/fame, job, world, page, limit | `RankingsData`: entries and nullable total; scoped character view |
...
- `server/src/server/RankingWorker.java:73–75`: current ranking eligibility is `c.gm=0 AND a.banned=0 AND c.level>=30`; ordering is level DESC, exp DESC, fame DESC, prior rank ASC. Adopt that predicate in the public view; a fame sort is an explicit alternate ordering. Final stable tie order should be source-reviewed and documented. Internal account linkage is required for filtering, never returned.
- The existing worker groups `job/100` and skips unsupported groups (`RankingWorker.java:88`); ensure custom classes are represented deliberately.
