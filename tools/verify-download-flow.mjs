...
try {
  for (const width of [1440, 390, 320]) {
    ...
    const download = page.getByRole('link', { name: 'DOWNLOAD LAUNCHER', exact: true });
    ...
    const full = page.getByRole('link', { name: 'DOWNLOAD FULL CLIENT', exact: true });
    assert.equal(await full.count(), release.fullClient ? 1 : 0);
    if (release.fullClient) assert.equal(await full.getAttribute('href'), release.fullClient.url);
    ...
    assert.doesNotMatch(main, /QA|diagnostic|debug/);
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth + 1), false);
    await page.getByText('SHA-256 checksum', { exact: true }).first().click();
